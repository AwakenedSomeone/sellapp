...
// 花括号里面的因为不是default方法，所以要加花括号，并且括号里可以写不止一个函数名
    ...
    export default {
    	...
    	created() {
    		this.$root.eventHub.$on('ratingtype.select', (type) => {
    			this._setType(type);//接收并执行一个函数
    		});
    		this.$root.eventHub.$on('content.toggle', (content) => {
    			this._setContent(content);//接收并执行一个函数
    		});
    	 },
    	methods: {
    		_setContent (content) {
    		...
    		addFirst (event) {
    			...
    			Vue.set(this.food,'count',1);// 第一次时没有就创建一个count
    		},
    		hide () {
    			this.showFlag = false;
    		},
...
@import "../../common/stylus/mixin.styl"
  .food
    ...
    .image-header
      ...
      width:100%
      height:0
      padding-top: 100%// 保证上下的padding值和盒子宽度一样大
      img
        position:absolute
        top:0
        left:0
      ...
      .back
        ...
        .icon-arrow_lift
          ...
          font-size:20px
          color:#fff
          text-shadow:0 0 2px rgba(7,17,27,0.6) //避免遇到白色背景图标看不见
    .content
      position:relative
      padding:18px
      .title
